			...
			struct stats_record *stats_prev)
{
	...
	long long int KiloBytes; /* KiloBytes per sec */
	double Mbps; /* Mega bits per sec */
	...
	/* Assignment#2: Print other XDP actions stats  */
	{
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			...
			" period:%f\n";
		//TODO some kind of loop through the actions and array
		for(int i = 0; i < XDP_ACTION_MAX; i++){
			...
			printf(fmt, action, rec->total.rx_packets, pps, KiloBytes, Mbps, period);
		}
	}
}
			  ...
			  struct stats_record *stats_rec)
{
	...
	map_collect(map_fd, map_type, XDP_ABORTED, &stats_rec->stats[0]);
	map_collect(map_fd, map_type, XDP_DROP, &stats_rec->stats[1]);
	map_collect(map_fd, map_type, XDP_PASS, &stats_rec->stats[2]);
	map_collect(map_fd, map_type, XDP_TX, &stats_rec->stats[3]);
	map_collect(map_fd, map_type, XDP_REDIRECT, &stats_rec->stats[4]);
}
